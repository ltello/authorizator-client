...
describe 'The response of the Authorizator to a request to access its api, differs depending on the request,
  ...
  let(:invalid_access_token_error_code_response) {double(:error => double(:code => invalid_access_token_error_codes.first))}
  ...
  let(:no_invalid_access_token_error_response)   {double(:error => double(:code => 7), :headers => {})}
  ...
  context '#invalid_access_token_response?(resp):' do
    ...
    it '... or the www-authenticate response header matches one of them.' do
